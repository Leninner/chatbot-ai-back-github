...
class SupabaseVectorStore(VectorStore):
    ...
    def similarity_search_with_relevance_scores(
        self, query: str, k: int = 4, client_id: int = 1, **kwargs: Any
    ) -> List[Tuple[Document, float]]:
        vectors = self._embedding.embed_documents([query])
        return self.similarity_search_by_vector_with_relevance_scores(vectors[0], k, client_id)

    def similarity_search_by_vector_with_relevance_scores(
        self, query: List[float], k: int
    ) -> List[Tuple[Document, float]]:
        match_documents_params = dict(query_embedding=query, match_count=k, client_id=self.client_id)
    ...
    def similarity_search_by_vector_returning_embeddings(
        self, query: List[float], k: int, client_id: int = 1, **kwargs: Any
    ) -> List[Tuple[Document, float, np.ndarray[np.float32, Any]]]:
        print(client_id, 'client_id:from similarity_search_by_vector_returning_embeddings')
        match_documents_params = dict(query_embedding=query, client_id=client_id, match_count=k)
        res = self._client.rpc(self.query_name, match_documents_params).execute()
    ...
    async def max_marginal_relevance_search_by_vector(
        ...
        **kwargs: Any,
        ...
        """
        result = await self.similarity_search_by_vector_returning_embeddings(
            embedding, fetch_k
        )
